...
size_t find_depth(const binary_tree_t *node) 
{
    ...
    if (node->parent)
	    ...
	    return (0);
    /* Depth of current node is 1 + parent's depth */
}
...
const binary_tree_t *find_first_leaf(const binary_tree_t *tree) 
{
    ...
    {
        return (find_first_leaf(tree->right)); /* Check right subtree if left is NULL */
    }
}
...
int check_perfect_recursive(const binary_tree_t *node, size_t leaf_depth, size_t level) 
{
    if (node == NULL) 
    {
...
int  binary_tree_is_perfect(const binary_tree_t *tree) 
{
    ...
    /* Find depth of any leaf (all leaves have the same depth in a perfect tree) */
    leaf_depth = find_depth(find_first_leaf(tree));

    /* Check if all nodes have both children and are at the same level as leaves */
